...
from cvar_psha.methods.mc import run_mc
...
from cvar_psha.methods.tree_mc import run_tree_mc
...
def run_1node(cfg: dict, config_path: Path | None = None) -> dict:
    ...
    print(f"  q*   = {np.array2string(gt.q_star, precision=4)}")
    ...
    for name, runs in method_runs.items():
        ...
        print(f"  {name:12s}  q={np.array2string(mean_q, precision=4)}  CVaR~{last_cvar:.4f}")
    print(f"  {'q*':12s}  q={np.array2string(gt.q_star, precision=4)}")
...
def run_3node(cfg: dict, config_path: Path | None = None) -> dict:
    ...
    print("Computing 3-node ground truth (path-mixture MC)...")
    ...
    for i in top:
        print(f"  q*[{gt.path_labels[i]}] = {gt.q_star[i]:.4f}")
    ...
    print(f"  {'q*':14s}  top={gt.path_labels[top_star]} ({gt.q_star[top_star]:.3f})")
